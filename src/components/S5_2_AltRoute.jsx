...
import { formatGuideDistance, getCompassDotPosition } from '../utils/geo';
...
function S5_2_AltRoute() {
  ...
  const dotPosition = useMemo(
    () => getCompassDotPosition(
      compass.centerX, compass.centerY,
      compass.dotOrbitRadius, destinationAngle,
      compass.dotSize, compass.dotSize,
    ),
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: BG, overflow: 'hidden' }}>
      ...
      <div style={{ ...abs(s5.innerRing), borderRadius: s5.innerRing.radius, background: '#FFFFFF1A' }} />

      {/* 목적지 점 */}
      <div aria-hidden style={{
        position: 'absolute',
        left: dotPosition.left, top: dotPosition.top,
        width: compass.dotSize, height: compass.dotSize,
        borderRadius: '50%', background: '#FFFFFF',
        transition: 'left 0.18s ease-out, top 0.18s ease-out',
        opacity: compassOpacity, pointerEvents: 'none',
      }} />

      {/* 방향 화살표 */}
      <div aria-hidden style={{ ...abs(s5.innerRing), borderRadius: '50%', overflow: 'hidden', pointerEvents: 'none' }}>
        <div style={{
          position: 'absolute', left: innerLocalCenter, top: innerLocalCenter,
          width: 0, height: 0,
          ...
          <div style={{
            ...
            <S5NavigationArrow
              ...
              strokeWidth={arrow.strokeWidth} color={arrow.color}
            />
          </div>
        </div>
      </div>
